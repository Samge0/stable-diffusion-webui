...
clothing_descriptions_new = ["collared shirt", "Camisole", "Blouse", "Sweater", "Cardigan", "Tank top", "Halter top", "Crop top", "Tunic top", "Peplum top", "Wrap top", "Off-the-shoulder top", "One-shoulder top", "Kimono top", "Bell sleeve top", "Bow tie top", "Lace top", "Sheer top", "Embellished top", "Ruffle top", "Wrap blouse", "Tie-neck blouse", "V-neck blouse", "Mock-neck blouse", "Collared blouse", "Crossover blouse", "Bow tie blouse", "Sleeveless blouse", "Balloon sleeve blouse", "Striped blouse"]
...
posture_verbs = [    "Sashaying",    "Gliding",    "Dancing",    "Grazing",    "Pirouetting",    "Striding",    "Glimmering",    "Twirling",    "Swaying",    "Languishing",    "Sweeping",    "Strolling",    "Slinking",    "Glowing",    "Fluttering",    "Posing",    "Sauntering",    "Musing",    "Radiating",    "Tiptoeing"]
...
def get_scenery() -> str:
    return random.choice(scenery_descriptions_new + scenery_descriptions_top30_world + scenery_descriptions_top30_ziran)
...
def get_prompt_dict(is_man: False) -> dict:
    scenery = get_scenery()
    clothing = get_clothing()
    _time = get_time()
    posture = get_posture()
    lora = get_lora()
    u_log.print_log(f"当前获取的随机参数：场景：{scenery}， 服装：{clothing}， 时间：{_time}， 姿态：{posture}， lora：{lora}")
